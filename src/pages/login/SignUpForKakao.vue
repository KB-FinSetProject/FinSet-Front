    ...
    <div class="col-12">
      ...
      <div class="input-group">
        <input
          ...
          class="form-control"
          id="id"
          placeholder="hongildong@kb.com"
          v-model="member.id"
        />
        <button class="btn btn-custom" type="button" @click="checkId">
          인증하기
        </button>
      </div>
      <span
    ...
    <div class="col-12">
      ...
      <div class="input-group">
        <input
          ...
          required
        />
        <button class="btn btn-custom" type="button" @click="checkName">
          중복 확인
        </button>
        <br />
      </div>
      <span :class="disableSubmitName.value ? 'text-primary' : 'text-danger'">{{
        checkErrorName
      }}</span>
...
const checkName = async () => {
  if (!member.name) {
    ...
    console.log(disableSubmitName.value, typeof disableSubmitName.value);
    checkErrorName.value = disableSubmitName.value
      ? '이미 사용중인 닉네임 입니다'
      : '사용가능한 닉네임 입니다.';
  }
};
...
onMounted(async () => {
  if (route.query.code != null) {
    const data = await loginApi.getKakaoInfo(route.query.code);
    console.log('@@@@');
    console.log(data);

    member.id = data.email;
